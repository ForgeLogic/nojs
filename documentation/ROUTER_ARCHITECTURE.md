...
- Call the `onChange` callback to trigger rendering
...
**Important**: Route handlers are called on **every navigation** to create fresh component instances. This ensures clean state for each route.
...
Located in `runtime/renderer.go`:
...
9. **Router**: Call `onChange(component)`
10. **Renderer**: Call `SetCurrentComponent()`
...
7. **Router.handlePathChange()**: Call `onChange(newComponent)`
8. **Renderer**: Call `SetCurrentComponent()` and `ReRender()`
