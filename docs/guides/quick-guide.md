...
> **New to nojs?** Start with the [Getting Started guide](../getting-started.md) to scaffold a new project, then come back here as a feature reference.
...
### OnMount — run once before first render {#onmount--run-once-before-first-render}
...
### OnParametersSet — run before every render (including first) {#onparametersset--run-before-every-render-including-first}
...
### OnUnmount — run once when removed from the tree {#onunmount--run-once-when-removed-from-the-tree}
